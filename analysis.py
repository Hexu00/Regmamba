...
file_dir = 'Results/'
file_name = ['affine', 'NiftyReg_IXI', 'deedsBCV_IXI', 'Vxm_1_ncc_1_diffusion_1', 'Vxm_2_ncc_1_diffusion_1','MIDIR_ncc_1_diffusion_1',
             'TransMorph_ncc_1_diffusion_1','TransMorphBSpline_ncc_1_diffusion_1','RegMamba','RegMamba-diff']
...
deeds  = plt.boxplot(all_data[2].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor-sep*3, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops, patch_artist=True)
vxm1= plt.boxplot(all_data[3].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor-sep*2, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
vxm2 = plt.boxplot(all_data[4].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor-sep, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
midir = plt.boxplot(all_data[5].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
trans = plt.boxplot(all_data[6].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor+sep, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
transSp = plt.boxplot(all_data[7].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor+sep*2, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
regmamba = plt.boxplot(all_data[8].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor+sep*3, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
regmambadiff = plt.boxplot(all_data[9].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor+sep*4, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops,patch_artist=True)
set_box_color(affine, 'plum') # colors are from http://colorbrewer2.org/
set_box_color(nifty, 'slateblue')
set_box_color(deeds, 'tan')
set_box_color(vxm1, 'wheat')
set_box_color(vxm2, 'lightgreen')
set_box_color(midir, 'peachpuff')
set_box_color(trans, 'olive')
set_box_color(transSp, 'sandybrown')
set_box_color(regmamba, 'pink')
set_box_color(regmambadiff, 'lightcoral')
plt.grid(linestyle='--', linewidth=1)
plt.plot([], c='plum', label='Affine')
plt.plot([], c='slateblue', label='NiftyReg')
plt.plot([], c='tan', label='deedsBCV')
plt.plot([], c='wheat', label='VoxelMorph-1')
plt.plot([], c='lightgreen', label='VoxelMorph-2')
plt.plot([], c='peachpuff', label='MIDIR')
plt.plot([], c='olive', label='TransMorph')
plt.plot([], c='sandybrown', label='TransMorph-bspl')
plt.plot([], c='pink', label='RegMamba')
plt.plot([], c='lightcoral', label='RegMamba-diff')
